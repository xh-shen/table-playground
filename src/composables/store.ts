...
import stableCode from '../template/stable.js?raw'
...
export type VersionKey = 'vue' | 'elementPlus' | 'stable'
...
export const useStore = (initial: Initial) => {
  ...
  watch(
    ...
    (version) => {
      const file = new File(
        STABLE_FILE,
        generateSTableCode(version, userOptions.styleSource).trim(),
        hideFile
      )
      state.files[STABLE_FILE] = file
      compileFile(store, file)
    },
    { immediate: true }
  )

  function generateSTableCode(version: string, styleSource?: string) {
    const style = styleSource
      ? styleSource.replace('#VERSION#', version)
      : genCdnLink('@shene/table', version, '/dist/index.css')
    return stableCode.replace('#STYLE#', style)
  }
  ...
  function setSTableVersion(version: string) {
